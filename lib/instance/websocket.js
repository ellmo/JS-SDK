...
function _deviceSendData (data, cb) {
  var params = null
  if (isPlainObject(data) && data.type === 'datapoint') { // 设置数据端点
    params = {
      appid: this.userid,
      name: data.name || '',
      datapoint: data.data
...
function _deviceDisconnect () {
  ...
  // 从sdk设备列表中删除对象
  _.remove(this._sdk.devices, function (item) {
    item.id = self.id
  })
}
...
function _recvDeviceData (device, data) {
  if (device) {
    ...
    if (data.type === 'base64') {
      let decodedArr = base64.decode(data.data)
      device._fire(deviceEvent.DATA, params, decodedArr)
      return
    }
